    ...
    execute(message) {
         ...
         */
            function listMajors(auth) {
                ...
                spreadsheetId: '1EeTqkbwk4Y5wRvQTa_kqpKtxR_mUZL74cHJ0-61gpcg',
                range: 'A2:D',
                }, (err, res) => {
                if (err) return console.log('The API returned an error: ' + err);
                const rows = res.data.values;
                if (rows.length) {
                    message.channel.send('First Name, Last Name, Points:');
                    // Print columns A and E, which correspond to indices 0 and 4.
                    rows.map((row) => {
                    message.channel.send(`${row[0]}, ${row[1]}, ${row[3]}`);
                    });
                } else {
